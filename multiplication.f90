    MODULE CARP
	!IMPLICIT NONE
  PUBLIC::DGEMMD,DGEMVD ,XERBLA
   CONTAINS     
	
		  SUBROUTINE DGEMMD ( M, N, K, A,  B, C )

!     .. Scalar Arguments ..
      CHARACTER(1),PARAMETER::TRANSA="N"
	  CHARACTER(1),PARAMETER::TRANSB="N"
      INTEGER,INTENT(IN):: M, N, K
	  INTEGER LDA, LDB, LDC	 ! ASAGIDA
...
!  =======
!
!  DGEMMD  performs one of the matrix-matrix operations
!
...
!
      NOTA  = LSAMED( TRANSA, 'N' )
      NOTB  = LSAMED( TRANSB, 'N' )
      IF( NOTA )THEN
         NROWA = M
         NCOLA = K
      ELSE
...
!
      INFO = 0
      IF(      ( .NOT.NOTA                 ).AND.( .NOT.LSAMED( TRANSA, 'C' ) ).AND.( .NOT.LSAMED( TRANSA, 'T' ) )      )THEN
         INFO = 1
      ELSE IF( ( .NOT.NOTB                 ).AND.( .NOT.LSAMED( TRANSB, 'C' ) ).AND.( .NOT.LSAMED( TRANSB, 'T' ) )      )THEN
         INFO = 2
      ELSE IF( M  .LT.0               )THEN
         INFO = 3
      ELSE IF( N  .LT.0               )THEN
...
!     End of DGEMMD .
!
      END SUBROUTINE DGEMMD

	
	  SUBROUTINE DGEMVD ( M, N, A, X,Y)
!     .. Scalar Arguments ..
...
!  =======
!
!  DGEMVD  performs one of the matrix-vector operations
!
...
LDA=M
      ...
      INFO = 0
      IF     ( .NOT.LSAMED( TRANS, 'N' ).AND..NOT.LSAMED( TRANS, 'T' ).AND. .NOT.LSAMED( TRANS, 'C' )      )THEN
         INFO = 1
      ELSE IF( M.LT.0 )THEN
         INFO = 2
      ELSE IF( N.LT.0 )THEN
...
!
      ...
      RETURN
!
 9999 FORMAT( ' *** XERBLA was called from ', A6, ' with INFO = ', I6,' instead of ', I2, ' ***' )
 9998 FORMAT( ' *** XERBLA was called with SRNAME = ', A6,' instead of ', A6, ' ***' )
 9997 FORMAT( ' *** On entry to ', A6, ' parameter number ', I6,' had an illegal value ***' )
!
!     End of XERBLA
...
!  =======
!
!  LSAMED returns .TRUE. if CA is the same letter as CB regardless of
!  case.
!
!  Arguments
!  =========
